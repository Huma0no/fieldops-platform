...
| commission_rate | real | Percentage of gross withheld as commission (the house's cut) — the technician nets the remainder. Defaults to 20 when a technician is added; editable at any time, independently per technician — not a global constant. Not applicable to `role = "owner"`. |
...
- `role = "owner"` keeps 100% of their own income, no commission withheld. `role = "technician"` (non-owner, described as "collaborator" in the business context) nets `gross × (1 − commission_rate)` — there is no single fixed split across all technicians. Full compensation model: `/docs/dispatch/LEDGER-SPEC.md`.
...
| commission_retained | real | Dollar amount withheld — `gross_amount × commission_rate_applied/100`, stored alongside the rate as a convenience so the API doesn't recompute it on every read |
| net_amount | real | Amount technician receives — gross_amount × (1 − commission_rate_applied), ± this technician's own adjustment lines below |
| excluded | boolean | Default false. Dispatcher can exclude a technician from a given run without losing their underlying data — flips back in when unmarked. **Not yet built** — no `excluded` column or exclude action exists in code as of 2026-07-30. |

**Rules:**
- `role = "owner"` has no `pay_period_lines` row — the owner isn't paid out of their own Ledger, they keep whatever remains after all technician payouts and `ghost_deduction`. **Known gap:** as of 2026-07-30 the running code gives the owner their own line — confirm and reconcile before treating this rule as built.
- `net_amount` = `gross_amount × (1 − commission_rate_applied)` plus the sum of that technician's own adjustment lines (see `pay_period_adjustments`) — never affected by `ghost_deduction`.
