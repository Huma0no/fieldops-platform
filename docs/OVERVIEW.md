...
For the current reading order, use `/docs/ARCHITECTURE-MAP.md` for source-of-truth boundaries and journeys, then `/docs/OFFLINE-FIRST-CONTRACT.md` for FieldOps local-work and delivery boundaries, then `/docs/DOMAIN-CONTRACT-INDEX.md` for entity contracts, then the owning feature/domain specifications.
